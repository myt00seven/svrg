...
def main(model=MODEL,gradient = GRADIENT, n_epochs=NUM_EPOCHS, n_hidden = NUM_HIDDEN_UNITS):
    ...
    if gradient == "stream" or gradient == "all": # It is StreamingSVRG
        models.update({ 'streaming': (custom_streaming_svrg1, {'learning_rate': 0.01, 'm': 50, 'k_s_0': 1.0, 'k_s_ratio':1.03, 'adaptive': False, 'adaptive_half_life_period':20, 'ada_factor':ada_factor}) })
